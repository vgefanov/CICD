...
void get_grep_flags(int argc, char **argv, int *flags, template *template,
                    int *templates_var) {
  int e = 0, f = 0;
  for (int i = 1; i < argc; i++) {
    if (strspn(argv[i], "-")) {
      ...
      if (strchr(argv[i], 'f')) {
        f++;
        template[*templates_var].file_name =
            get_template_file_name(argc, argv, i);
...
void scan_files(int argc, char **argv, template *template, int templates_var,
                int *flags) {
...
int var_files(int argc, char **argv, template *template, int *flags) {
  ...
  int same = 0;
  for (int i = 1; i < argc; i++)
    if (is_arg_a_file(i, argv, template, &same, flags))
      res++;
  return res;
}

int is_arg_a_file(int i, char **argv, template *template, int *same,
                  const int *flags) {
  return (!strspn(argv[i], "-") &&
          !((strchr(argv[i], 'f') && strspn(argv[i], "-")) ||
            ...
            (i != 1 && argv[i - 1][strlen(argv[i - 1]) - 1] == 'e' &&
             strspn(argv[i - 1], "-")) ||
            (!(flags[8] || flags[0]) && !strcmp(argv[i], template[0].name) &&
             ((*same)--) >= 0)));
}
...
void get_files(int argc, char **argv, template *template, file_struct *files,
               int *flags) {
  int same = 0;
  int k = 0;
  for (int i = 1; i < argc; i++) {
...
void files_processing(template *template, int templates_var, file_struct *files,
                      int files_var, int *flags) {
  ...
  for (int i = 0; i < files_var; i++) {
    ...
    if (fp) {
      if (empty_line) {
        ...
        maching_lines = seek_in_file(fp, template, templates_var, flags,
                                     files + i, files_var);
      }
      fclose(fp);
    } else {
    ...
    if (flags[C])
      print_files_c(files + i, files_var, maching_lines, flags);
    if (flags[L])
      print_files_l(files + i, maching_lines);
  }
}
...
int is_empty_line_in_templates(template *template, int templates_var) {
  ...
  for (int i = 0; i < templates_var; i++) {
    if (template[i].is_file) {
      ...
      if (is_empty_line(template[i].name))
        res = 1;
    }
  }
  return res;
...
int seek_in_file(FILE *fp, template *template, int templates_var, int *flags,
                 file_struct *files, int files_var) {
  ...
  while (getline(&line, &size, fp) != -1) {
    if (line) {
      ...
      for (int i = 0; i < templates_var; i++) {
        ...
        seek_choice(line, template + i, files, flags, &matching_lines,
                    files_var, line_number, &one_time_print, &file_name_print);
      }
      if (flags[V] && one_time_print == 1) {
        print_match(line, flags, files, files_var, line_number, &one_time_print,
                    &matching_lines);
      }
    }
  }
...
void seek_choice(char *line, template *template, file_struct *files, int *flags,
                 int *matching_lines, int files_var, int line_number,
                 int *one_time_print, int *file_name_print) {
  if (flags[O] && !flags[C] && !flags[V]) {
    if (template->is_file) {
      seek_file_templates(line, template, files, flags, matching_lines,
                          files_var, line_number, one_time_print,
                          file_name_print);
    } else {
      seek_o_template(line, template, files, flags, matching_lines, files_var,
                      line_number, file_name_print);
    }
  } else {
    if (template->is_file) {
      seek_file_templates(line, template, files, flags, matching_lines,
      ...
      seek_template(line, *template, files, flags, matching_lines, files_var,
                    line_number, one_time_print);
    }
  }
}

void seek_file_templates(char *line, template *template, file_struct *files,
                         int *flags, int *matching_lines, int files_var,
  ...
  if (fp) {
    ...
    while (getline(&file_line, &file_line_size, fp) != -1) {
      if (line) {
        ...
        if (flags[O] && !flags[C] && !flags[V]) {
          seek_o_template(line, template, files, flags, matching_lines,
                          files_var, line_number, file_name_print);
        } else {
          seek_template(line, *template, files, flags, matching_lines,
                        files_var, line_number, one_time_print);
        }
      }
    }
...
void seek_o_template(char *line, template *template, file_struct *files,
                     int *flags, int *maching_lines, int files_var,
                     int line_number, int *file_name_print) {
  regex_t regex;
  ...
  if (!comp_val) {
    ...
    while (regexec(&regex, line + offset, 1, &match, eflags) == 0) {
      if (*file_name_print)
        (*maching_lines)++;
      eflags = REG_NOTBOL;
      print_o_match(file_name_print, files, offset, match, line, flags,
                    line_number, files_var);
      offset += match.rm_eo;
...
void print_o_match(int *file_name_print, file_struct *files, size_t offset,
...
void seek_template(char *line, template template, file_struct *files,
                   int *flags, int *matching_lines, int files_var,
                   int line_number, int *one_time_print) {
  regex_t regex;
  trim_n(template.name);
...
void print_match(char *line, int *flags, file_struct *files, int files_var,
...
void print_files_c(file_struct *files, int files_var, int maching_lines,
                   const int *flags) {
  if (!flags[H] && files_var > 1)
    printf("%s:", files->file_name);
  if (!flags[L])
